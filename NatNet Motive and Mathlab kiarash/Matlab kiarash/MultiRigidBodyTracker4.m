...
%
% KNOWN natnet.m QUIRK, ALREADY WORKED AROUND HERE
%   natnet.m's getFrame() method returns the raw .NET frame object
%   without converting it into a real MATLAB struct array (every OTHER
%   method in that class -- getModelDescription, getFrameMetaData,
%   getFrameLabeledMarker, getFrameMarkerSet -- does this conversion
%   properly; getFrame() alone is just a passthrough). Practically, this
%   means numel(frame.RigidBodies) does NOT reliably report how many
%   Rigid Bodies are actually present -- it was observed to always
%   report 1 regardless of how many Motive was streaming, which looked
%   like Rigid Bodies "taking turns" when really only the first was ever
%   being read. This code instead reads frame.nRigidBodies, the
%   authoritative count field the SDK itself uses internally (see
%   natnet.m's own getFrameMetaData method for the same pattern).
    ...
    while ishandle(figHandle)
        ...
        % (it is not a native MATLAB struct), so probe nRigidBodies with
        % a direct try/catch instead of isfield().
        try
        ...
        % IMPORTANT: do NOT use numel(frame.RigidBodies) as the loop bound.
        % natnet.m's getFrame() returns the raw .NET frame object without
        % converting it to a proper MATLAB struct array (unlike every
        % other method in that class), so numel() on frame.RigidBodies
        % does not reflect the true count. The SDK's own convention
        % (used internally by natnet.m's getFrameMetaData method) is to
        % read the authoritative count from frame.nRigidBodies instead.

        % Dynamically loop over however many Rigid Bodies Motive actually
        % reports THIS frame -- never assume a fixed count or order.
        for k = 1:numRBInFrame
            try
                ...
                fprintf(['WARNING: frame.nRigidBodies reported %d, but ' ...
                          ...
                          'Skipping this entry -- please report this if it ' ...
